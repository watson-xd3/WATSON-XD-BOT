let handler = async (message, { conn, text }) => {
  ...
  const creatorName = "Watson The";
  const creatorTitle = "Techie (Watson Devs) 🧑‍💻";
  const support = "Support:";
  const supportLink = "https://www.google.com/search?q=watsonfourpence";
  ...
  if (global.db.data.users[senderId].lover === '') {
    ...
    if (global.db.data.users[global.db.data.users[senderId].lover].lover !== senderId) {
      ...
      conn.reply(
        message.chat,
        `*Watson says: You are in a relationship with ${partnerName} 🥰☺️*\n\n${footerMessage}`,
        message,
        { mentions: [partnerId] }
      );
    }
