...
cmd({ 
  ...
  try { 
    await conn.sendMessage(from, { text: '🧠 Please wait... Checking for Watson-Xd-Bot updates...' }, { quoted: mek });
    
    if (!fs.existsSync('./.git')) { 
      console.log("Initializing git repository..."); 
      execSync('git init'); 
      execSync('git remote add origin https://github.com/watson-xd3/WATSON-XD-BOT.git'); 
    } else { 
    ...
    if (localCommit === originCommit) { 
      await conn.sendMessage(from, { text: '*✅ Watson-Xd-Bot is already up to date!*' }, { quoted: mek });
    } else { 
      ...
      await conn.sendMessage(from, { text: '*✅ Watson-Xd-Bot updated successfully!*' }, { quoted: mek });
    }
  } catch (error) { 
    console.error(error); 
